...
def process_favor_sim_results(books_data_path):
    ...
    # %%
    favorSimResults = booker["FavorSimResults"]
    favorSimResults.drop()

    dbUserTagFeatures = booker["UserTagFeatures"]

    userTagFeatures = dbUserTagFeatures.find()

    print("reading user tag features")
    user_id = list()
    user_features = list()
    for elem in userTagFeatures:
        print("uid:", elem["uid"])
